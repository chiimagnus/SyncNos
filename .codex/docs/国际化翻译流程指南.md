...
> 可选：如果你觉得 `Resource/Localizable.xcstrings` 太大难以维护，可以用 `.codex/scripts/xcstrings_split.py` 拆分到多个小文件，再用 `.codex/scripts/xcstrings_merge.py` 合并回单一文件（仍使用默认 Localizable 表，避免改动 Swift 调用）。
...
## 可选：拆分成多个 xcstrings（更易管理）

推荐使用“拆分目录维护 + 合并生成单一 `Resource/Localizable.xcstrings`”的方式：

```bash
# 1) 拆分（默认按 key 的第一个分段，例如 settings.saved -> settings）
python3 .codex/scripts/xcstrings_split.py --compact

# 2) 合并回单一 Localizable（默认检查重复 key）
...
位置：`.codex/scripts/compact_xcstrings.py`
...
# 方式 2：指定自定义路径
python3 .codex/scripts/compact_xcstrings.py path/to/file.xcstrings

# 方式 3：直接执行
./.codex/scripts/compact_xcstrings.py
```
...
| **格式压缩脚本** | `.codex/scripts/compact_xcstrings.py` |
| **拆分脚本** | `.codex/scripts/xcstrings_split.py` |
| **合并脚本** | `.codex/scripts/xcstrings_merge.py` |
| **本指南** | `.codex/docs/国际化翻译流程指南.md` |
