...
如果 DetailView 需要响应特殊的键盘快捷键（如 WechatChat 的 Option+方向键），**不要使用 SwiftUI 的 `@FocusState` + `.onKeyPress()`**，因为它与 MainListView 的 AppKit firstResponder 管理冲突。
...
> **参考**: `WechatChatDetailView.swift` 和 `MainListView.swift` 中的 Option+方向键处理
