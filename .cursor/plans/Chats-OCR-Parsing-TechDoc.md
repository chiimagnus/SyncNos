...
### Step 5.5：居中系统/时间戳（X 轴中间）的处理（绝对阈值检测）✅ 2025-12-30 更新

在私聊中，时间戳与系统提示通常位于 X 轴中间（居中灰字）。

#### 5.5.1 原算法问题（K-Means 聚类）

原算法使用两步 K-Means 聚类来识别系统消息，但存在**聚类边界效应**问题：

- 聚类是**相对判断**，依赖所有候选消息的分布
- 即使两个时间戳位置完全相同（如 centerX ≈ 0.51），聚类边界可能刚好落在它们中间
- 导致一个时间戳被正确识别，另一个却被误判为气泡消息

#### 5.5.2 新算法：绝对阈值检测

抛弃聚类方法，改用**绝对几何特征检测**，独立判断每条候选消息：

```
对于每个候选消息的 bbox，计算：
- left = minX / imageWidth           // 左侧留白比例
- right = 1 - maxX / imageWidth      // 右侧留白比例
- centerX = (left + right) / 2 + left // 中心点位置
- bias = left - right                // 左右偏置
- relativeWidth = bbox.width / imageWidth  // 相对宽度
```

**核心判定条件**（四个条件全部满足才标记为系统消息）：

| 条件 | 阈值 | 含义 |
|-----|------|------|
| `abs(centerX - 0.5) <= 0.10` | ±10% | 中心点接近图片中心 |
| `abs(bias) <= 0.10` | ±10% | 左右留白平衡 |
| `min(left, right) >= 0.25` | ≥25% | 两侧都有明显留白 |
| `relativeWidth < 0.50` | <50% | 宽度较窄 |

**扩展检测**（处理聚类遗漏）：

- 如果存在明显的左右气泡分布（bias 范围 > 0.5）
- 对于 `bias` 绝对值 < 0.05 且 `centerX` 接近 0.5 的候选，额外标记为系统消息

#### 5.5.3 示例验证

假设图像宽度 = 3000px，两个时间戳的 bbox：

| 时间戳 | x | width | centerX | bias | left | right | relativeWidth | 判定 |
|-------|---|-------|---------|------|------|-------|---------------|------|
| "6月30日 11:19" | 1446 | 168 | 0.51 | 0.02 | 0.48 | 0.46 | 0.06 | ✅ 系统 |
| "6月30日 11:30" | 1446 | 172 | 0.51 | 0.02 | 0.48 | 0.46 | 0.06 | ✅ 系统 |

两个时间戳都满足所有条件，都被正确标记为系统消息。

#### 5.5.4 优势对比

| 对比项 | 原算法 (K-Means 聚类) | 新算法 (绝对阈值) |
|--------|----------------------|------------------|
| **判断方式** | 相对于其他消息 | 独立判断每条消息 |
| **稳定性** | 受其他消息分布影响 | 不受影响 |
| **边界情况** | 容易出错 | 稳定可预测 |
| **阈值** | 动态计算 | 固定、可调 |
| **可解释性** | 需理解聚类过程 | 直接读取阈值 |

UI 上将 `.system` 以居中灰底的样式展示（类似微信），同时不影响左右气泡消息的方向判定。
