...
- 弹窗 UI（`src/ui/popup/*`）：聊天列表选择、导出菜单（JSON/Markdown）、Notion 连接与父页面选择的设置页。
  - 按业务拆分为多文件：`popup-core.js`（共享能力）、`popup-tabs.js`、`popup-list.js`、`popup-chat-preview.js`（Chats hover/focus 预览弹层）、`popup-export.js`、`popup-notion.js`、`popup-about.js`、`popup.js`（初始化编排）。
