...
- 构建 Firefox 包（会压缩/混淆 `dist-firefox/*.js`，并生成 `.xpi`）：`npm --prefix Extensions/WebClipper run build:firefox`
- 校验 Firefox 产物（不做源码语法检查）：`npm --prefix Extensions/WebClipper run check:dist:firefox`
- 生成 AMO 源码包（用于 reviewer 要求的 Source code 上传）：`npm --prefix Extensions/WebClipper run package:amo-source`

AMO Source Package（Reviewer 要求的源码包说明）：
...
- 备注：构建会读取 `node_modules/markdown-it/dist/markdown-it.js`（上游未压缩 bundle）并打包进 `dist-firefox/popup.js`
...
- 提交的 `.xpi` 包含 terser 压缩后的产物时，AMO 通常会要求同时上传可复现构建的源码包（本项目用 `package:amo-source` 生成）
- Firefox 产物 manifest 使用 `background.scripts` 作为兼容回退（Firefox 会忽略 `background.service_worker`）
- Firefox 产物 manifest 注入 `browser_specific_settings.gecko.data_collection_permissions`（AMO 要求），默认 `required: ["none"]`
- 默认 `strict_min_version` 为 `142.0`（与 `data_collection_permissions` 的支持版本对齐）
