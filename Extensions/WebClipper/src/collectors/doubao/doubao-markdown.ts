...
const NS: any = collectorContext as any;
  ...
  function detectCodeLanguage(preEl: any): any {
    ...
    const codeBlock = preEl.closest(".code-block-element-R6c8c0,.custom-code-block-container");
    if (codeBlock && codeBlock.querySelector) {
      const title = codeBlock.querySelector(".title-TXcgFG .text-OkYU_0");
      const text = normalizeCodeLanguage(title && title.textContent ? title.textContent : "");
      if (text) return text;
    }
  ...
  function removeNonContentNodes(container: any): any {
    ...
    const classSkips = [
      ...
      ".actions-XLKatx",
      ".action-btn-NhM0gh",
      ".header-wrapper-Mbk8s6",
      ".action-ysQCxz",
      ".hoverable-kRHiX2",
      ".mask-wrapper-ujeSdZ",
      ".mask-lrkkzM",
      ".container-Uxvbjy.md-box-line-break"
    ];
  ...
  function getAssistantContentRoot(wrapper: any): any {
    ...
    return (
      wrapper.querySelector("[data-testid='message_text_content']") ||
      wrapper.querySelector(".flow-markdown-body") ||
      wrapper
    );
  }
  ...
  function htmlToMarkdown(root: any): any {
    ...
    function renderNode(node: any, ctx: any): any {
      ...
      if (/md-box-line-break|table-header-qH9Ajf|header-wrapper-Mbk8s6|mask-wrapper-ujeSdZ/.test(classText)) return "";
      ...
      if (tag === "div" && /\bparagraph-element\b|\bparagraph-pP9ZLC\b/.test(classText)) {
        const text = normalizeMarkdown(renderChildren(node, ctx));
        return text ? `${text}\n\n` : "";
      }
