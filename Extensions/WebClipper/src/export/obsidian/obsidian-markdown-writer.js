(function () {
  ...
  function yamlEscapeString(value) {
    ...
    // Minimal YAML string escaping (double-quoted).
  ...
  function buildMessageChunk(message) {
    const m = message || {};
  ...
  function buildFullNoteMarkdown({ conversation, messages, syncnosObject }) {
  ...
  function buildIncrementalAppendMarkdown({ newMessages }) {
    return buildMessagesMarkdown(newMessages);
  }

  async function appendUnderMessagesHeading({ client, filePath, markdown }) {
    // @ts-ignore
  ...
  async function replaceSyncnosFrontmatter({ client, filePath, syncnosObject }) {
    // @ts-ignore
    return client.patchVaultFile(filePath, {
      operation: "replace",
