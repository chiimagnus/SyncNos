...
(function () {
  ...
  if (split && splitToggle) {
    ...
    for (var q = 0; q < menuItems.length; q++) {
      if (menuItems[q].getAttribute('data-menu-store') === bkey) menuItems[q].style.display = 'none';
    }
    function closeSplit() {
  ...
  function captureZh() {
    for (var i = 0; i < langNodes.length; i++) {
      if (langNodes[i].getAttribute('data-zh') === null) langNodes[i].setAttribute('data-zh', langNodes[i].innerHTML);
    }
  }
