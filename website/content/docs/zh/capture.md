...
SyncNos 提供三项浏览器快捷操作：**打开 SyncNos Popup**、**保存当前页面**、**打开/聚焦 SyncNos 应用**。

默认不分配任何快捷键，需要在浏览器自己的扩展快捷键设置中绑定。你可以在 SyncNos 的 **设置 → 通用 → 快捷键** 中查看浏览器当前实际使用的键位；浏览器允许直接进入快捷键管理页时会显示**管理快捷键**，否则请按照卡片中的手动配置提示操作。

**保存当前页面**与 Popup 使用相同的页面采集能力。当前页面不支持采集时，快捷键会明确提示错误，不会绕过既有的页面支持范围。
