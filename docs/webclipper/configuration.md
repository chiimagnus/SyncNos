...
配置的当前真源是 `wxt.config.ts`（manifest/权限）、根 `package.json`（命令/依赖）、各 service protocol（值的归一化）和 settings ViewModel（读写编排）。不要在文档复制版本号、权限列表或完整 storage key 清单。
...
manifest、权限、发布构建或产物相关改动再运行 `npm run gate`。可用的 browser-specific `dev:*` / `build:*` 命令以根 `package.json` 为准。
