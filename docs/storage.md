...
| `active` / `native:*` | `~/.syncnos/syncnos.sqlite`（通过 SyncNos Native Host） | list/detail/tail/search/comments/mapping/capture 等 facts operation 只走 SQLite；浏览器不得因 Host/FTS 错误回退到旧 IDB facts。验证激活与 profile reference rebase 完成后，旧 IDB facts 才按迁移流程清理。 |
