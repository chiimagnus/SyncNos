...
| `chrome.storage.local` | WebClipper | 本地 KV | Notion / Obsidian / Feishu（OAuth + DocX 路径 + job status）/ `inpage_display_mode` / `ai_chat_auto_save_enabled` / `ai_chat_cache_images_enabled` / `web_article_cache_images_enabled` / `anti_hotlink_rules_v1` / `markdown_reading_profile_v1` / Chat with AI 等运行设置（不缓存 Insight 统计结果） | `src/viewmodels/settings/useSettingsSceneController.ts`、settings stores |
...
| 存储面 | 字段 / 键 | 用途 | 备注 |
| --- | --- | --- | --- |
| IndexedDB `sync_mappings` | `feishuDocId`, `feishuLastContentHash` | 绑定“本地 conversation ↔ 飞书 DocX”，并用于 `skipped_unchanged` | 若 DocX 被删除/不可访问，会创建新 DocX 并更新 mapping |
| IndexedDB `conversations` | `feishuDocId?` | UI “Open in Feishu” 的快速数据源 | 同步写 mapping 时会回填到 conversation；备份导入时也会从 mapping 回填 |
| `chrome.storage.local` | `feishu_sync_job_v1` | 同步 job snapshot（运行中/完成/失败/中止） | background reload 会把遗留的 running job 标记为 aborted，避免 UI 永久“同步中” |
| `chrome.storage.local` | `feishu_chat_folder` / `feishu_article_folder` / `feishu_video_folder` | 三类会话的目标路径（相对云盘根目录） | 默认分别是 `SyncNos-AIChats` / `SyncNos-WebArticles` / `SyncNos-Videos`；不存在会自动创建 |
| `chrome.storage.local` | `feishu_oauth_token_v1` | OAuth token（access/refresh/expiresAt） | **敏感键**：不会进入 Zip v2 备份 |

- `backup-utils.ts` 会排除 `notion_oauth_token_v1` / `feishu_oauth_token_v1`、`notion_oauth_client_secret` / `feishu_oauth_client_secret`，并排除任何以 `notion_oauth_token` / `feishu_oauth_token` 开头的键。
