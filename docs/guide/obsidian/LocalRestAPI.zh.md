...
## 第 1 步：安装并启用 Obsidian Local REST API
...
## 第 2 步：启用 Insecure HTTP（WebClipper 当前版本必须）
...
## 第 3 步：复制 API Key，并粘贴到 WebClipper
...
- WebClipper 写入 Obsidian 的文件名格式为 `<source>-<title>-<stableId10>.md`；当标题变化时会自动重命名（写新文件并清理旧文件）。
