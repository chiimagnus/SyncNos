...
export function ConversationDetailPane({
  ...
  const floatingIslandClassName = [
    ...
    'tw-bg-[color-mix(in_srgb,var(--bg-card)_88%,transparent)] tw-backdrop-blur-[10px] tw-backdrop-saturate-150',
    'tw-shadow-[0_8px_24px_rgba(0,0,0,0.14)]',
  ].join(' ');
      ...
      <section className="tw-flex tw-flex-col tw-bg-[var(--bg-card)]" aria-label={t('conversationDetailAria')}>
        ...
        >
          {onExpandSidebar || onBack ? (
            <div
              className={[floatingIslandClassName, 'tw-absolute tw-left-3 tw-top-2 md:tw-left-4'].join(' ')}
              data-detail-nav-island="true"
            >
              {onExpandSidebar ? (
                <button
                  type="button"
          ...
          {copyActions.length > 0 || canOpenCommentsSidebar || hasMoreMenuContent ? (
            <div
              className={[
                floatingIslandClassName,
                'tw-absolute tw-right-3 tw-top-2 tw-whitespace-nowrap md:tw-right-4',
              ].join(' ')}
              data-detail-actions-island="true"
            >
