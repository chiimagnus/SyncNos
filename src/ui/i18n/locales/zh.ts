...
export const zh: { [K in TranslationKey]: string } = {
  ...
  section_videos_label: '视频采集',
  section_videos_desc: '支持的视频页面与抓取教程。',
  ...
  localCliIntegrationHeading: '本地 CLI 集成',
  localCliIntegrationLabel: '启用 SyncNos CLI',
  localCliIntegrationHint: '浏览器运行时，允许本机 syncnos 命令使用这个浏览器 Profile 中的 SyncNos。',
  localCliIntegrationUnavailable: '当前浏览器不支持 Native Messaging。',
  ...
  contextMenuSaveCurrentVideoTranscript: '保存视频',
  ...
  chatgptApiCaptureAdvancedImages:
    '对话图片与图片下载分开保存。是否自动缓存到本地由 AI 图片缓存开关决定；关闭时，未缓存的 ChatGPT 图片仍可按需解析。普通工具截图和视觉执行产物不会作为对话图片保存。',
  ...
  videosSectionHeading: '视频采集',
  videosSectionIntro:
    '保存 YouTube / Bilibili 视频页面的可用信息；有字幕时保存字幕，Bilibili 播放器提供章节/看点时一并保存（不会下载视频）。',
  ...
  videosSectionHowToStep2:
    '如需同时保存字幕，开启字幕/CC并切换到所需轨道/语言（B 站 AI 字幕需要在字幕菜单里选择 AI 字幕轨道）。',
  videosSectionHowToStep3: '字幕出现后再保存可记录字幕；没有字幕也可以直接保存简介、章节等可用视频信息。',
  videosSectionHowToStep4: '在页面空白处右键 → SyncNos → 保存视频。',
  videosSectionTroubleshootingHeading: '常见问题',
  videosSectionTroubleshootingNoSubtitles:
    '如果提示“未检测到字幕；已保存可用的视频信息。”，说明本次没有可信字幕；简介、章节等可用信息仍会保存。字幕稍后加载后再次保存即可补充或更新字幕。',
  videosSectionTroubleshootingUnsupported: '仅支持上面列出的 URL 类型；Bilibili av、YouTube Shorts 等目前不支持。',
  ...
  exportButton: '导出',
  exportOptions: '导出选项',
  tooltipExportDetailed: '导出已选内容',
  tooltipExportSelectFirstDetailed: '请先勾选内容，再执行导出',
  ...
  insightVideosSectionAria: '视频',
  insightVideosSectionTitle: '🎬 视频',
  ...
  // Popup sync nudge
  popupSyncNudgeTitle: '建议打开标签页版进行同步',
  popupSyncNudgeDontShowAria: '后续不再弹出此提示',
  popupSyncNudgeDontShowLabel: '后续不再弹出此提示弹窗',
  popupSyncNudgeDismiss: '暂不前往',
  popupSyncNudgeConfirm: '了解并前往',
