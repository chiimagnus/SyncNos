...
- 请求限流：为 WeRead 和 Dedao 实现 API 请求限流和重试机制，增强反爬处理能力

*功能改进*

- 缓存服务：简化 WeRead 和 Dedao 缓存服务的模型容器管理，改进错误处理
- 文档：更新 README 和 CLAUDE.md，添加 Dedao 支持说明

## v0.8.8.4 2025年12月3日

*新增功能*

- 快捷键：支持 cmd+4 快捷键切换到 Dedao 数据源
- 登录回调：Dedao 和 WeRead 视图模型添加登录成功回调，自动刷新 UI 和加载书籍

*功能改进*

- 线程安全：同步逻辑确保通知在主线程派发，提升 UI 响应一致性
- 同步引擎：NotionSyncEngine 更优雅地处理空高亮情况，确保时间戳正确更新
- 代码规范：移除 MainListView 中冗余的通知发送，遵循单一入口原则

## v0.8.8.3 2025年12月3日

*功能改进*

- 弹窗统一：集中 Notion 配置和会话过期提醒到 MainListView，减少视图模型中的重复弹窗逻辑

## v0.8.8.2 2025年12月2日

*新增功能*

- 高亮分页：GoodLinksViewModel 添加高亮分页支持，提升大量数据时的加载体验
- 同步状态：DedaoDetailView 和 WeReadDetailView 添加外部同步状态处理和错误提示
- 存储隔离：为 WeRead 和 Dedao 实现独立模型容器存储，避免数据冲突

*功能改进*

- UI 优化：批量更新策略减少不必要的 UI 刷新，提升性能
- 布局管理：添加底部栏高度管理，防止内容重叠
- 缓存策略：DedaoCacheService 高亮管理采用完整替换策略，确保数据完整性
- 任务管理：DedaoDetailViewModel 支持任务取消和增强的错误处理

## v0.8.8.1 2025年12月2日

*新增功能*

- Dedao 集成：完整的得到电子书数据源支持
  - WebView 登录：通过网页视图实现得到账号登录
  - 令牌桶限流：防止触发反爬机制的请求限流器
  - 本地缓存：SwiftData 缓存书籍和高亮数据
  - 自动同步：集成自动同步服务
- 日志搜索：LogWindow 添加搜索功能，支持在消息、文件名、函数名中搜索

*功能改进*

- Dedao 模型：更新模型支持可选字段和 effectiveId，增强数据处理灵活性
- Dedao 认证：优化认证逻辑，过渡到 WebView 登录并增强 Cookie 验证
- UI 清理：移除 LogWindow 背景色，简化界面设计

## v0.8.8 2025年12月2日

*功能改进*

- AboutView：增强布局和功能，添加更多链接入口

## v0.8.7.1 2025年11月28日

*功能改进*

- 懒加载：实现 WeRead cookie header 懒加载，防止启动时触发 Keychain 权限提示
- 代码重构：简化 DataSourceSwitchViewModel，移除 UserDefaults 观察者，改用显式注入
- 配置修复：更新调试引导标志默认值为 false
