...
final class EPUBContextService {
    ...
    func loadContext(booksDBPath: String, assetId: String, cfi: String?, highlightText: String) -> HighlightContextTriple? {
        do {
            guard let epubPath = try fetchBookPath(booksDBPath: booksDBPath, assetId: assetId) else {
            ...
            logger.error("EPUBContextService: Error occurred while loading context: \(error)")
            return nil
        }
    }

    // MARK: - DB: fetch ZPATH
    private func fetchBookPath(booksDBPath: String, assetId: String) throws -> String? {
        var db: OpaquePointer?
        let rc = sqlite3_open_v2(booksDBPath, &db, SQLITE_OPEN_READONLY, nil)
        guard rc == SQLITE_OK, let handle = db else {
            logger.error("EPUBContextService: Failed to open database at \(booksDBPath)")
            throw NSError(domain: "EPUBContextService", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to open database"])
        }
        ...
        guard sqlite3_prepare_v2(handle, sql, -1, &stmt, nil) == SQLITE_OK else {
            logger.error("EPUBContextService: Failed to prepare statement: \(sql)")
            throw NSError(domain: "EPUBContextService", code: 2, userInfo: [NSLocalizedDescriptionKey: "Failed to prepare SQL statement"])
        }
