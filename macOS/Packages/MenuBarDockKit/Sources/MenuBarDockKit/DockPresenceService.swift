...
@MainActor
public enum DockPresenceService {
    /// 在即将展示主窗口时调用：必要时显示 Dock，并激活应用。
    public static func prepareForPresentingMainWindow() {
    ...
    public static func hideDockIfAllowedWhenNoVisibleWindows() {
        guard AppIconDisplayMode.current == .menuBarOnly else { return }

        let app = NSApplication.shared
        let hasVisibleWindows = app.windows.contains(where: { window in
            window.isVisible && (window.canBecomeMain || window.styleMask.contains(.fullScreen))
        })
