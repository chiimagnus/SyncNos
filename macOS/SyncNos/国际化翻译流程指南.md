...
sed -i '' 's/"目标字符串" : {/"目标字符串" : {\
      "localizations" : {\
        ...
        ... 其他语言 ...\
      },/' macOS/Resource/Localizable.xcstrings

# 步骤 2: 验证
python3 -m json.tool macOS/Resource/Localizable.xcstrings > /dev/null && echo "✅ JSON valid"
```
...
```bash
grep -n '"目标字符串"' macOS/Resource/Localizable.xcstrings
```

**示例：**
```bash
grep -n '"Connect Your Notion"' macOS/Resource/Localizable.xcstrings
...
```bash
sed -n '行号,行号+20p' macOS/Resource/Localizable.xcstrings
```
...
# 项目构建
xcodebuild -project macOS/SyncNos.xcodeproj -scheme SyncNos -configuration Debug build -quiet
```
...
```bash
cd /path/to/SyncNos && \
sed -i '' 's/"String1" : {/.../' macOS/Resource/Localizable.xcstrings && \
sed -i '' 's/"String2" : {/.../' macOS/Resource/Localizable.xcstrings
```
...
4. 查看详细错误：
   ```bash
   python3 -m json.tool macOS/Resource/Localizable.xcstrings
   ```
...
```bash
git checkout -- macOS/Resource/Localizable.xcstrings
```
