...
final class DedaoAutoSyncProvider: AutoSyncSourceProvider {
    ...
    private func syncAllBooksSmart() async throws {
        ...
        let sortKey: BookListSortKey = {
            guard let raw = UserDefaults.standard.string(forKey: ListSortPreferenceKeys.Dedao.sortKey),
                  let k = BookListSortKey(rawValue: raw) else {
                return .title
            }
            return k
        }()
        let sortAscending = UserDefaults.standard.object(forKey: ListSortPreferenceKeys.Dedao.sortAscending) as? Bool ?? true
