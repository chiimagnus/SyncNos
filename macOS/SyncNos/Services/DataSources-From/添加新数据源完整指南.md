...
| 认证/会话 | `XxxAuthService.swift`（OAuth/API Key 等）或 `SiteLoginsStore`（Cookie） | `Account/AuthService.swift`、`Services/SiteLogins/SiteLoginsStore.swift` |
...
| 文件 | 枚举 | 说明 |
|------|------|------|
| `Models/Core/Models.swift` | `ContentSource` | 统一内容来源标识（同步队列和 UI 共用） |
| `Models/Core/HighlightColorScheme.swift` | `HighlightSource` | 高亮颜色映射 |

> **注意**: `SyncSource` 枚举已被移除，统一使用 `ContentSource`。
> 
> **参考**: 搜索现有的 `weRead` case 查看添加位置

### 1.1.1 创建 DataSourceUIProvider（P3-P4 协议驱动重构）

**文件**: `macOS/SyncNos/Models/DataSourceProviders/XxxUIProvider.swift`

实现 `DataSourceUIProvider` 协议，为新数据源提供 UI 配置：

```swift
import SwiftUI

struct XxxUIProvider: DataSourceUIProvider {
    let source: ContentSource = .xxx
    let displayName = "Xxx"
    let iconName = "book"  // SF Symbol
    var brandColor: Color { Color("BrandXxx") }
    
    let filterChangedNotification: Notification.Name = .xxxFilterChanged
    
    let hasFilterMenu = true
    let supportsHighlightColors = true
    let supportsSync = true
    let highlightSource: HighlightSource = .xxx
    
    let menuTitle: LocalizedStringKey = "Books"
    let enabledStorageKey = "datasource.xxx.enabled"
    let defaultEnabled = false
    let highlightColorTheme: HighlightColorTheme? = .xxx

    @MainActor
    func makeSettingsView() -> AnyView {
        AnyView(XxxSettingsView())
    }
}
```

### 1.1.2 注册到 DataSourceRegistry（P5）

**文件**: `macOS/SyncNos/Models/Core/DataSourceRegistry.swift`
...
private init() {
    // ... 现有数据源 ...
    register(XxxUIProvider())
}
```

> **参考**: 查看 `DataSourceUIProvider.swift` 和各 `*UIProvider.swift` 了解协议详情

### 1.2 创建数据模型文件

**文件**: `macOS/SyncNos/Models/XxxModels.swift`
...
**文件**: `macOS/SyncNos/Models/UnifiedHighlight.swift`
...
**文件**: `macOS/SyncNos/Models/XxxCacheModels.swift`
...
- [ ] 在 `ContentSource` 枚举中添加新数据源（`Models/Core/Models.swift`）
- [ ] 在 `HighlightSource` 枚举中添加新数据源
- [ ] 在 `HighlightColorScheme.allDefinitions` 中添加颜色映射
- [ ] 在 `HighlightColorTheme` 枚举中添加新 case（`Views/Components/Controls/FilterSortBar.swift`）
- [ ] 创建 `XxxModels.swift`
- [ ] 在 `UnifiedHighlight.swift` 中添加转换初始化器
- [ ] **[P3-P5] 创建 `XxxUIProvider.swift` 并注册到 `DataSourceRegistry`**
...
```bash
mkdir -p macOS/SyncNos/Services/DataSources-From/Xxx
```

### 2.2 在 Protocols.swift 中添加协议定义

**文件**: `macOS/SyncNos/Services/Core/Protocols.swift`

添加服务协议：
- `XxxAuthServiceProtocol`（如需 OAuth/API Key 等“非 Cookie”认证）
- `XxxAPIServiceProtocol` 或 `XxxDatabaseServiceProtocol`
- `XxxCacheServiceProtocol`（可选）

Cookie 登录（WebView 抓 cookies）不再使用各数据源的 `XxxAuthService`，统一使用：
- `SiteLoginsStoreProtocol`（domain → cookieHeader）
- `CookieWebLoginSheet`（通用登录 UI）
...
| 数据源类型 | 需要创建的服务 | 参考 |
|-----------|---------------|------|
| Web API（OAuth/API Key） | `XxxAuthService.swift`, `XxxAPIService.swift` | `Account/` 或现有 OAuth 实现 |
| Web API（Cookie 登录） | `XxxAPIService.swift` + `SiteLoginsStore` + `CookieWebLoginSheet` | `WeRead/`、`Dedao/`、`Services/SiteLogins/` |
...
- [ ] （如需 OAuth/API Key）创建 `XxxAuthService.swift`
- [ ] 创建 `XxxAPIService.swift` 或 `XxxDatabaseService.swift`
- [ ] （如需 Cookie 登录）接入 `SiteLoginsStore` + `CookieWebLoginSheet`
...
**文件**: `macOS/SyncNos/Services/DataSources-To/Notion/Sync/Adapters/XxxNotionAdapter.swift`
...
```bash
mkdir -p macOS/SyncNos/ViewModels/Xxx
```
...
```bash
mkdir -p macOS/SyncNos/Views/Xxx
```
...
| `XxxSettingsView.swift` | 设置视图（在 `Views/Settings/SyncFrom/`） | ✅ |
| `XxxLoginView.swift` | 登录视图（在 `Views/Settings/SyncFrom/`） | 如需 WebView 登录 |
...
// 注意：使用 ContentSource.listFocusRequestedNotification（定义在 NotificationNames.swift）
.onReceive(NotificationCenter.default.publisher(for: ContentSource.xxx.listFocusRequestedNotification).receive(on: DispatchQueue.main)) { _ in
...
if hasOption && !hasCommand && !hasControl {
    if self.contentSource == .xxx && self.keyboardNavigationTarget == .detail {
        ...
        case 126: // Option+↑
            // 注意：如需添加新的通知，请在 NotificationNames.swift 中统一定义
...
// 使用统一定义的通知名称（需先在 NotificationNames.swift 中定义 .xxxNavigateUp）
.onReceive(NotificationCenter.default.publisher(for: .xxxNavigateUp)) { _ in
    // 执行操作
}
```
...
// 注意：选中项不再需要单独的 @State 变量！
// 使用统一的 SelectionState 管理（已存在于 MainListView 中）：
// @State private var selectionState = SelectionState()
```

> **P6 重构说明**: 选择状态现在由 `SelectionState` 类统一管理。
> 访问选中项：`selectionState.selection(for: .xxx)`
> 设置选中项：`selectionState.setSelection(for: .xxx, ids: [...])`
> 获取绑定：`selectionState.selectionBinding(for: .xxx)`
>
> **参考**: 搜索 `selectionState` 查看使用方式
...
选择状态由 `SelectionState` 统一管理，不需要额外代码。
`SelectionState` 会自动处理所有数据源的选择状态。

> **注意**: 使用 `selectionState.clear(for: .xxx)` 清除单个数据源，
> 或 `selectionState.clearAll()` 清除所有选择。
...
使用 `DataSourceFilterSections` 通用组件：
...
var xxxFilterMenu: some View {
    ...
    DataSourceFilterSections<BookListSortKey>(
        filterNotification: .xxxFilterChanged,  // 需先在 NotificationNames.swift 中定义
        sortKey: Binding(
            get: { xxxVM.sortKey },
            set: { xxxVM.sortKey = $0 }
        ),
        sortAscending: Binding(
            get: { xxxVM.sortAscending },
            set: { xxxVM.sortAscending = $0 }
        )
    ) {
        // 可选：添加额外的筛选按钮
        VMFilterToggleButton(
            title: "Some Filter",
            isOn: xxxVM.someFilter,
            action: { xxxVM.someFilter.toggle() },
            notificationName: .xxxFilterChanged,
            userInfoKey: "someFilter"
        )
    }
}
```
...
> **P8 重构说明**: 使用 `DataSourceFilterSections<SortKey>` 通用组件替代手写的排序菜单。
> 组件自动处理：
> - 排序键选择和状态显示
> - 升序/降序切换
> - 发送 `filterChangedNotification`
> 
> **参考**: `Views/Components/Controls/DataSourceFilterMenu.swift`
...
✅ **不再需要修改 SettingsView**（Settings 侧边栏与详情页由 `DataSourceRegistry` + `DataSourceUIProvider` 自动生成）。

你只需要：
1. 创建 `XxxSettingsView.swift`
2. 在 `XxxUIProvider.makeSettingsView()` 返回 `AnyView(XxxSettingsView())`
3. 将 `XxxUIProvider` 注册到 `DataSourceRegistry`

> **备注（会话过期）**：当前架构里 session 过期由 `MainListView` 统一弹窗提示，用户点“Go to Login”会直接打开主界面的登录 Sheet（不跳转到 Settings）。
...
- [ ] 创建 `XxxListView.swift`
  ...
  - [ ] 添加 `.onAppear` 延迟获取焦点
  - [ ] 添加 `.onReceive(.xxxListFocusRequested)` 通知监听（需先在 NotificationNames.swift 中定义）
- [ ] （可选）创建 `XxxDetailView.swift`
  - [ ] 添加 `onScrollViewResolved: (NSScrollView) -> Void` 参数
  - [ ] 使用 `EnclosingScrollViewReader` 解析 NSScrollView
- [ ] 创建 `Views/Settings/SyncFrom/XxxSettingsView.swift`
...
- [ ] 在 `XxxUIProvider.makeSettingsView()` 中返回 `XxxSettingsView`（Settings 自动接入）
...
**文件**: `macOS/SyncNos/Services/SyncScheduling/XxxAutoSyncProvider.swift`
...
**文件**: `macOS/SyncNos/Services/SyncScheduling/AutoSyncService.swift`
...
**文件**: `macOS/SyncNos/Services/Core/DIContainer.swift`
...
```bash
xcodebuild -project macOS/SyncNos.xcodeproj -scheme SyncNos -configuration Debug build 2>&1 | grep -E "error:|warning:"
```
...
| 路径 | 必需 |
|------|------|
| `Models/DataSourceProviders/XxxUIProvider.swift` | ✅ |
| `Models/XxxModels.swift` | ✅ |
| `Models/XxxCacheModels.swift` | 可选 |
| `Services/DataSources-From/Xxx/XxxAuthService.swift` | 如需 OAuth/API Key 等“非 Cookie”认证 |
...
| `Services/DataSources-To/Notion/Sync/Adapters/XxxNotionAdapter.swift` | ✅ |
| `Services/DataSources-To/Notion/Sync/Adapters/XxxSyncService.swift` | 可选 |
...
| `Views/Settings/SyncFrom/XxxSettingsView.swift` | ✅ |
| `Views/Settings/SyncFrom/XxxLoginView.swift` | 如需登录（Cookie 登录建议复用 `CookieWebLoginSheet`） |
...
| 路径 | 修改内容 |
|------|----------|
| `Models/Core/Models.swift` | 添加 `ContentSource.xxx` |
| `Models/Core/DataSourceRegistry.swift` | 注册 `XxxUIProvider` |
...
| `macOS/Resource/Localizable.xcstrings` | 添加本地化字符串 |
...
| `ShowSessionExpiredAlert` | 会话过期提示（由 MainListView 统一弹窗） |
...
*最后更新: 2026-01-22*
*更新内容:*
- *P2: 移除 SyncSource 枚举，统一使用 ContentSource*
- *P3-P5: 添加 DataSourceUIProvider 协议和 DataSourceRegistry 注册表*
- *P6: SelectionState 统一选择状态管理*
- *P8: DataSourceFilterSections 通用筛选菜单组件*
- *P9: Settings 侧边栏/总览由 DataSourceUIProvider 驱动*
