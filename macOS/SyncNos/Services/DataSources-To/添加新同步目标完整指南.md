...
当前实现与 Notion 强耦合（此处以 Notion 作为示例目标）：

```
ViewModel → NotionSyncEngine → NotionClient
              ↓
...
```bash
mkdir -p macOS/SyncNos/Services/SyncEngine
```

### 1.2 创建 SyncTargetProtocol

**文件**: `macOS/SyncNos/Services/SyncEngine/SyncTargetProtocol.swift`
...
> **参考**: 现有 `NotionSyncSourceProtocol.swift` 的协议设计模式（路径见下）

### 1.3 移动并重命名 SyncSourceProtocol

**从**: `Services/DataSources-To/Notion/Sync/NotionSyncSourceProtocol.swift`
...
**从**: `Services/DataSources-To/Notion/Sync/SyncTimestampStore.swift`
...
**文件**: `macOS/SyncNos/Services/SyncEngine/SyncTargetRegistry.swift`
...
**文件**: `macOS/SyncNos/Services/DataSources-To/Notion/NotionSyncTarget.swift`
...
```bash
mkdir -p macOS/SyncNos/Services/DataSources-To/Yyy
```
...
**文件**: `macOS/SyncNos/Services/DataSources-To/Yyy/YyyConfigStore.swift`
...
```bash
# 创建服务文件
touch macOS/SyncNos/Services/DataSources-To/Yyy/YyyService.swift
touch macOS/SyncNos/Services/DataSources-To/Yyy/YyyRequestHelper.swift
```
...
```bash
# 创建文件写入服务
touch macOS/SyncNos/Services/DataSources-To/Yyy/YyyFileWriter.swift
touch macOS/SyncNos/Services/DataSources-To/Yyy/YyyMarkdownFormatter.swift
```

### 2.5 创建同步目标实现

**文件**: `macOS/SyncNos/Services/DataSources-To/Yyy/YyySyncTarget.swift`
...
**文件**: `macOS/SyncNos/Views/Settting/SyncTo/YyyIntegrationView.swift`
...
**文件**: `macOS/SyncNos/ViewModels/SyncTo/YyyIntegrationViewModel.swift`

### 3.3 更新设置视图

**文件**: `macOS/SyncNos/Views/Settting/General/SettingsView.swift`
...
> **参考**: `macOS/Resource/Localization/` 目录下的本地化文件
...
| 2.4 服务层 | `NotionClient.swift`, `NotionAPIClient.swift` | API 交互模式 |
...
```
Services/DataSources-To/Notion/Sync/Adapters/
...
Services/DataSources-To/Notion/Sync/
...
3. **错误重试**：参考 `NotionAPIClient.swift`
