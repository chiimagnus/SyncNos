...
SyncNos 提供「Display SyncNos icon」设置（见 `MenuBarDockKit.AppIconDisplayMode`、`ViewModels/Settings/AppIconDisplayViewModel.swift`），允许用户选择：
...
- 过滤窗口时使用 `isVisible && (canBecomeMain || styleMask.contains(.fullScreen))`
...
### 2) 为什么窗口过滤要用 `isVisible && (canBecomeMain || styleMask.contains(.fullScreen))`？

菜单栏 popover / 某些临时面板可能被 AppKit 视为 `NSWindow`，但它们不是我们要的“应用正在展示主界面窗口”。

使用 `canBecomeMain` 过滤能明显降低误判率；额外包含 `.fullScreen` 可避免全屏切换时误判“无窗口”。
