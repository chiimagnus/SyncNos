...
function detectCodeLanguage(preEl: any): any {
  ...
  if (codeBlock && codeBlock.querySelector) {
    const label = codeBlock.querySelector('.hyc-common-markdown__code__hd__l');
...
function getAssistantContentRoot(wrapper: any): any {
  ...
  return (
    ...
    wrapper.querySelector('.hyc-content-md .hyc-common-markdown') ||
    wrapper.querySelector('.hyc-content-md') ||
    wrapper
  );
}
...
function htmlToMarkdown(root: any): any {
  ...
  function renderNode(node: any, ctx: any): any {
    ...
    if (/md-box-line-break|hyc-common-markdown__code__hd|table-header-qH9Ajf/.test(classText)) return '';
    ...
    if (tag === 'span' && /\bhyc-common-markdown__link\b/.test(classText)) {
      const content = node.querySelector ? node.querySelector('.hyc-common-markdown__link__content') : null;
      const text = normalizeMarkdown((content && content.textContent) || node.textContent || '');
      return linkMarkdownFromSpan(node, text);
    }
    ...
    if (tag === 'div' && /\bybc-p\b/.test(classText)) {
      const text = normalizeMarkdown(renderChildren(node, ctx));
      return text ? `${text}\n\n` : '';
    }
