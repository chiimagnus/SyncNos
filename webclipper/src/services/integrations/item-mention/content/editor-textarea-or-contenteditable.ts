...
function findTextareaOrContentEditableEditor(): EditorHandle | null {
  const doc = document;
  if (!doc) return null;
  ...
  if (activeEl) {
    ...
    const closestEditable = (activeEl as any).closest
      ? ((activeEl as any).closest('[contenteditable="true"],[contenteditable="plaintext-only"]') as Element | null)
      : null;
    if (closestEditable && isContentEditable(closestEditable) && isVisible(closestEditable)) {
      return { kind: 'contenteditable', el: closestEditable as HTMLElement };
    }
  }
  ...
  const boxes = Array.from(
    doc.querySelectorAll?.(
      '[role="textbox"][contenteditable="true"], [contenteditable="true"][role="textbox"], [role="textbox"][contenteditable="plaintext-only"], [contenteditable="plaintext-only"][role="textbox"]',
    ) || [],
  ) as Element[];
...
export function createTextareaOrContentEditableEditorAdapter(): EditorAdapter {
  return {
    detectActiveEditor() {
      return findTextareaOrContentEditableEditor();
    },
