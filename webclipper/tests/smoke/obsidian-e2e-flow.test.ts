...
describe('obsidian local rest api sync e2e flow (mock)', () => {
  it('rebuilds notes on every sync, and handles auth failure', async () => {
    ...
    globalThis.fetch = async (_url: any, init: any) => {
      ...
      if (authFail) {
        return new Response(JSON.stringify({ errorCode: 40100, message: 'unauthorized' }), {
          status: 401,
      ...
      if (method === 'GET') {
        ...
        return new Response(JSON.stringify({ frontmatter: remoteFrontmatter || {}, content: remoteContent || '' }), {
          status: 200,
          headers: { 'content-type': 'application/json' },
        });
      }
