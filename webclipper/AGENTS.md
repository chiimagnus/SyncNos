...
- `webclipper/src/ui/**`：UI（popup/app/inpage），只放组件/样式/DOM 面板；设置页由 `src/ui/settings/SettingsScene.tsx` 统筹，窄屏走 `SettingsTopTabsNav.tsx`、宽屏走 `SettingsSidebarNav.tsx`，Inpage 相关设置集中在 `src/ui/settings/sections/InpageSection.tsx`，包括阅读风格和 anti-hotlink 域名编辑器
