...
describe('ConversationDetailPane header actions', () => {
  ...
  it('keeps the comments toggle in the right floating island and title in content metadata', async () => {
    ...
    const floatingLayer = document.querySelector('[data-detail-floating-layer="true"]') as HTMLElement | null;
    ...
    expect(outlineAnchor).toBeTruthy();
    expect(Boolean(metadata!.compareDocumentPosition(outlineAnchor!) & Node.DOCUMENT_POSITION_FOLLOWING)).toBe(true);
    expect(floatingLayer?.className).toContain('tw-z-40');
    expect(outlineAnchor?.className).toContain('tw-z-30');
