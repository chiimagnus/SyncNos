...
3. Settings / Conversations UI 改动时，不要只改单个组件；同时核对这些真源：
   ...
   - `webclipper/src/viewmodels/settings/useSettingsSceneController.ts`（`inpage_display_mode` / `ai_chat_auto_save_enabled` / `ai_chat_cache_images_enabled` / `chat_with_*`）
