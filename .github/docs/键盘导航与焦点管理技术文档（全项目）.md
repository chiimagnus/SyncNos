...
| 文件 | 职责 |
|------|------|
| `MainListView+KeyboardMonitor.swift` | 主窗口键盘事件监听 + Detail 滚动控制 |
| `MainListView+FocusManager.swift` | 主窗口 List/Detail 焦点切换（firstResponder 落点/恢复） |
| `Views/Commands/ViewCommands.swift` | 菜单快捷键：Cmd+1/2/…、⌥⌘←/→ 等（带窗口上下文隔离） |
| `Views/Commands/EditCommands.swift` | 菜单快捷键：⌘K 打开搜索面板（带窗口上下文隔离） |
| `Views/Commands/WindowCommandContext.swift` | FocusedSceneValue 上下文（类似 VSCode “when”） |
...
- `Views/Commands/WindowCommandContext.swift`
