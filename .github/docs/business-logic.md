...
```mermaid
flowchart TD
    A["用户配置 Notion 授权与 Parent Page"] --> B["连接数据源<br/>Apple Books / GoodLinks / WeRead / Dedao / Chats"]
    B --> C["选择条目并发起同步<br/>或 Auto Sync 触发"]
    C --> D["任务入队 + 并发限流 + 可取消"]
    D --> E["Adapter 拉取并归一化为 UnifiedHighlight"]
    E --> F{同步策略}
    F -->|Single Database| G["确保来源数据库存在 + 确保属性"]
    G --> H["确保条目页面存在"]
    H --> I["可选写入头部内容<br/>如 GoodLinks 正文"]
    I --> J["追加/更新高亮块"]
    F -->|Per-Book Apple Books| K["确保每本书数据库存在"]
    K --> L["创建/更新高亮条目"]
    J --> M["更新 Last Sync Time / Highlight Count"]
    L --> M
    M --> N["写入本地同步映射与时间戳"]
    N --> O["UI 展示成功/失败/进度"]
```
