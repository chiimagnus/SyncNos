...
| `video` conversation kind | WebClipper 的视频字幕会话分类；落点为 `SyncNos-Videos` | `conversation-kinds.ts`, `VideosSection.tsx` |
| `article_body` | article 会话的正文消息 key | `article-fetch.ts` |
| `article_comments` | article 详情页的本地评论线程 store；会随 Zip v2 备份 / 导入保留 | `src/services/comments/data/storage-idb.ts`, `ArticleCommentsSection.tsx`, `src/services/sync/backup/export.ts`, `src/services/sync/backup/import.ts` |
| `video_transcript` | 视频字幕消息的正文 key / 内容类型；可包含时间戳或纯文本字幕行 | `video-transcript-capture.ts`, `video-transcript-parse.ts` |
| `inpage_display_mode` | 控制 inpage UI 显示范围的开关（`supported / all / off`，并兼容旧 `inpage_supported_only`） | `src/services/bootstrap/content.ts`, Settings |
| `markdown_reading_profile_v1` | 控制 popup / app 详情页 markdown 阅读风格的设置键（`medium / notion / book`） | `src/ui/settings/sections/InpageSection.tsx`, `markdown-reading-profiles.ts` |
| `anti_hotlink_rules_v1` | article 图片反防盗链规则的本地映射键；命中后会自动补 referer 并走图片缓存链路 | `src/platform/webext/anti-hotlink-rules-store.ts`, `src/services/integrations/anti-hotlink/anti-hotlink-settings.ts` |
| `ai_chat_cache_images_enabled` / `web_article_cache_images_enabled` | 控制 chat/article 消息采集时是否尝试图片内联的设置键 | `src/viewmodels/settings/useSettingsSceneController.ts`, `src/services/conversations/background/handlers.ts`, `src/collectors/web/article-fetch.ts` |
| detail header `tools` slot | 会话详情头的工具动作槽位，与 `open` 并列；Chat with AI 与 cache-images 都在该槽位 | `src/services/integrations/detail-header-action-types.ts`, `DetailHeaderActionBar.tsx`, `chatwith-detail-header-actions.ts` |
| `cache-images` | conversation detail 中触发历史消息图片回填的工具动作 id | `src/viewmodels/conversations/conversations-context.tsx` |
| `OPEN_INPAGE_COMMENTS_PANEL` | 打开页面内评论面板的内容脚本消息 | `message-contracts.ts`, `src/services/bootstrap/inpage-comments-panel-content-handlers.ts` |
...
| `BACKFILL_CONVERSATION_IMAGES` | 前端触发历史消息图片回填的 CORE 消息类型 | `message-contracts.ts`, `src/services/conversations/background/handlers.ts` |
| `COMMENTS_MESSAGE_TYPES` | WebClipper 评论线程的后台消息类型集合 | `message-contracts.ts`, `src/services/comments/background/handlers.ts` |
| Zip v2 | 当前标准备份格式，包含 `article_comments` 与其它本地事实源 | `src/services/sync/backup/export.ts`, `src/services/sync/backup/import.ts` |
...
## 更新记录（Update Notes）
- 2026-04-18：补充 `video` conversation kind 与 `video_transcript` 术语，方便在 deepwiki 中统一描述视频字幕采集链路。
