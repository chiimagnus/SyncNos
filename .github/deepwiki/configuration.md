...
| `manifest.version` | `wxt.config.ts` | `1.5.4` | 商店 workflow 校验的版本事实源 |
...
## 更新记录（Update Notes）
- 2026-04-16：同步 `manifest.version` 到 `1.5.4`，并刷新版本事实源、发布校验与 deepwiki 元数据；保留 `DB_VERSION = 8`、Inpage / anti-hotlink / comments 相关事实不变。
- 2026-04-09：同步本页 `manifest.version` 单一事实源；补齐 `declarativeNetRequestWithHostAccess`、Notion OAuth worker / anti-hotlink host permissions 与 article comments / backup 相关说明，并对齐文章抓取的反防盗链图片自动缓存事实。
