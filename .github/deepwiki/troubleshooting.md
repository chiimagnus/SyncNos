...
| 会话图片一直是外链 / 缓存图片按钮无效 | `ai_chat_cache_images_enabled` / `web_article_cache_images_enabled` / `anti_hotlink_rules_v1` / detail tools / backfill job | `src/viewmodels/settings/useSettingsSceneController.ts`, `src/viewmodels/conversations/conversations-context.tsx`, `image-backfill-job.ts` | 先确认对应来源开关和 anti-hotlink 规则，再看路由消息和回填计数 |
...
- 如果用户改的是 `markdown_reading_profile_v1` 或 `anti_hotlink_rules_v1`，先确认当前页是否已经启动 content controller；未启动时看起来就会像“改了没生效”。
...
### 8. 会话图片缓存没有按预期生效
- 先确认对应来源开关：chat 使用 `ai_chat_cache_images_enabled`，article 使用 `web_article_cache_images_enabled`。
- 这些开关主要影响后续采集写入；如果是历史会话，需要在 detail header 手动触发 `cache-images` 才会回填。
- article 图片如果命中了 `anti_hotlink_rules_v1`，即使 `web_article_cache_images_enabled` 关闭也会自动走缓存链路；此时更应该检查规则是否能从 `chrome.storage.local` 正常读回。
...
| 在 detail 手动触发 `cache-images` | 历史消息图片仍是外链 | 完成后应看到更新计数并自动刷新 detail |
...
| `webclipper/src/viewmodels/conversations/conversations-context.tsx` | detail tools 动作注入与回调 | `cache-images` 是否被注入、是否触发 refresh 的第一现场 |
| `webclipper/src/services/conversations/background/handlers.ts` | 消息路由与图片内联开关 | 看 `ai_chat_cache_images_enabled` / `web_article_cache_images_enabled` 读取与 `BACKFILL_CONVERSATION_IMAGES` 注册 |
