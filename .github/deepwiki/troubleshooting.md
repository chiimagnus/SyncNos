...
| 会话图片一直是外链 / 缓存图片按钮无效 | `ai_chat_cache_images_enabled` / `web_article_cache_images_enabled` / detail tools / backfill job | `src/viewmodels/settings/useSettingsSceneController.ts`, `src/viewmodels/conversations/conversations-context.tsx`, `image-backfill-job.ts` | 先确认对应来源开关，再看路由消息和回填计数 |
