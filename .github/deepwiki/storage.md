...
| 存储层 | 名称 / 版本 | 结构 | 作用 |
| --- | --- | --- | --- |
| IndexedDB | `webclipper`, `DB_VERSION = 6` | `conversations`, `messages`, `sync_mappings`, `image_cache` | 扩展侧事实源（`image_cache` 仅作本地图片缓存，不改变会话主事实源） |
...
- 当前 Zip v2 备份覆盖 `conversations`、`messages`、`sync_mappings`、`image_cache`（已缓存的图片附件）以及 `storage-local`（非敏感键）。
