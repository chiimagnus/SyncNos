...
| Zip v2 备份 | WebClipper | 本地压缩包 | conversations CSV、分源 JSON、storage-local.json、image-cache、manifest | `backup/export.ts`, `backup/import.ts` |
...
| `article_comments` | object store | `canonicalUrl`, `conversationId`, `parentId`, `quoteText`, `quoteContext`, `commentText`, `createdAt`, `updatedAt` | article 详情页的本地评论线程、回复、删除与锚点定位 |
...
- `article_comments` 是独立的本地注释层：它会跟随 article 详情页和 inpage comments panel 使用，但**尚未进入 Zip v2 备份 / 导入**；如果你依赖恢复，这里是现成的 Coverage Gap。
...
| 产物 | 位置 / 目标 | 生成方 | 说明 |
| --- | --- | --- | --- |
| Zip v2 备份 | 用户本地 zip 文件 | WebClipper | 固定包含 `manifest.json`, `config/storage-local.json`, `sources/conversations.csv`, `sources/*.json`, `assets/image-cache/index.json`, `assets/image-cache/blobs/*` |
...
- 当前 Zip v2 备份覆盖 `conversations`、`messages`、`sync_mappings`、`image_cache`（已缓存的图片附件）以及 `storage-local`（非敏感键）；`article_comments` 目前仍未纳入，恢复后会丢失文章评论线程。
...
| Article comments store | `schema.ts`, `comments/data/storage-idb.ts` | `DB_VERSION = 7` 时补齐 `article_comments` store 与索引 | 文章评论线程、回复、删除、锚点定位 |
...
| 文章评论未随备份恢复 | WebClipper | `article_comments` 消失或只剩文章正文 | `comments/data/storage-idb.ts`, `backup/export.ts`, `backup/import.ts` |
