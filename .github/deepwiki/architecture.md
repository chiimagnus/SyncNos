...
| content | `webclipper/src/entrypoints/content.ts` | 注册 collectors、inpage UI、video transcript handlers、增量观察器、手动保存逻辑 | 采集稳定性、页面按钮体验 |
...
| `CONTENT_MESSAGE_TYPES.CAPTURE_VIDEO_TRANSCRIPT` | `webclipper/src/platform/messaging/message-contracts.ts` | video transcript content handlers、background bootstrap、VideosSection | 触发视频字幕采集与本地会话写入的 background -> content 指令，不复用 article fetch |
...
## 更新记录（Update Notes）
- 2026-04-18：补充视频字幕采集运行时、`CONTENT_MESSAGE_TYPES.CAPTURE_VIDEO_TRANSCRIPT` 契约、`VideosSection.tsx` 入口与相关源码引用；将视频字幕链路纳入架构修改热点。
