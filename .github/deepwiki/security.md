...
- 视频字幕采集复用现有 `host_permissions` 与 content script 运行时，不额外引入新的站点权限；只是在已支持的视频页上增加字幕拦截与 `video` 会话写入。
...
## 更新记录（Update Notes）
- 2026-04-18：补充视频字幕采集的权限边界说明，确认它不新增 host 权限，只复用现有 content script 运行时。
