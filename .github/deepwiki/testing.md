...
4. **WebClipper（配置）**：验证 Notion Parent Page、Obsidian connection test、备份导出 / 导入、`General` 分区里的 `inpage display mode / AI auto-save / AI $ mention / AI chat cache images` 设置行为，以及 `Inpage` 分区里的 `markdown reading profile / anti-hotlink` 行为；系统主题仍应随 `prefers-color-scheme` 切换。
