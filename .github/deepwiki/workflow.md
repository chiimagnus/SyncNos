...
| 7 | 视频字幕采集改动要同时检查 `src/entrypoints/video-transcript-interceptor.content.ts`、`src/entrypoints/video-transcript-bridge.content.ts`、`src/services/bootstrap/video-transcript-capture.ts`、`src/services/bootstrap/video-transcript-capture-content-handlers.ts`、`src/services/protocols/conversation-kinds.ts`、`src/ui/settings/sections/VideosSection.tsx` | 这条链路决定字幕拦截、`video` kind、`SyncNos-Videos` 和右键菜单说明是否一致 |
...
| 新增或调整视频字幕采集 | 同步更新 `modules/videos.md`、`modules/webclipper.md`、`data-flow.md`、`configuration.md`、`testing.md` | 避免视频字幕的入口、落点和验证步骤散落在多页 |
...
## 更新记录（Update Notes）
- 2026-04-18：补充视频字幕采集改动的工作流提醒，确保 `VideosSection.tsx`、`video` kind、`SyncNos-Videos` 和 deepwiki 路由页会一起同步。
